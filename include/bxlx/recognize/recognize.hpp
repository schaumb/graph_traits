...
namespace type_filter {
  ...
  struct tuple_t {
    ...
    constexpr static assert_types::reason<expected_tuple, T> valid() { return {}; }
  ...
  struct any {
    template<class>
    constexpr static std::true_type valid() { return {}; }
  };
  ...
  struct fix_typed_t {
    ...
    template<class T>
    constexpr static std::conditional_t<classification::classify<T> == t,
                                        std::true_type,
                                        assert_types::reason<assert_types::expected<t, assert_types::got<classification::classify<T>, T>>>> valid() { return {}; }
  };
...
namespace conditions {
  ...
  struct not_range_t : crtp_address_to_nullptr<not_range_t<Tr>>, condition_t {
  ...
  struct random_access_t : crtp_address_to_nullptr<random_access_t>, condition_t {
    template <class T, class>
    constexpr static std::conditional_t<
  ...
  struct not_multimap_t : crtp_address_to_nullptr<not_multimap_t>, condition_t {
    template <class T, class>
    constexpr static std::conditional_t<
  ...
  struct separator_of_t : crtp_address_to_nullptr<separator_of_t>, condition_t {
    template<class T, class>
    constexpr static auto valid() {
      if constexpr (type_traits::is_range_v<std::tuple_element_t<1, T>>) {
        if constexpr (std::is_base_of_v<std::random_access_iterator_tag, type_traits::range_iterator_tag_t<std::tuple_element_t<1, T>>>) {
          if constexpr (classification::classify<std::tuple_element_t<0, T>> == classification::type::index) {
            return std::true_type{};
          } else {
            return assert_types::reason<expected<classification::type::index, got<classification::classify<std::tuple_element_t<0, T>>>>>{};
          }
        } else if constexpr (std::is_same_v<std::tuple_element_t<0, T>, type_traits::detail::std_begin_t<std::add_const_t<std::tuple_element_t<1, T>>>>) {
          return std::true_type{};
        } else {
  ...
  struct no_t : crtp_address_to_nullptr<no_t>, condition_t {
  ...
  struct all {
    ...
    constexpr static auto valid() {
      if constexpr ((Ts::template valid<T, Props>() && ...)) {
        ...
        return std::tuple_element_t<0, decltype(std::tuple_cat(
                                             std::declval<std::conditional_t<Ts::template valid<T, Props>(), std::tuple<>, std::tuple<Ts>>>()...
  ...
  struct is_t : crtp_address_to_nullptr<is_t<K, V>>, condition_t {
    ...
    constexpr static auto valid() {
      if constexpr (Props{}.is_valid(properties::property<K, V>{})) {
        ...
        return assert_types::reason<assert_types::expected<&properties::property_v<K, V>>,
                                    assert_types::got<&properties::property_v<K, typename decltype(Props{}.template has_property<K>())::type>>>{};
      }
    }
  };

  template<class K>
  struct exclude_t : crtp_address_to_nullptr<exclude_t<K>>, condition_t {
    template<class, class Props>
    constexpr static auto valid() {
  ...
  constexpr auto operator&&(const all<Ts...>*, const U&)
        -> std::enable_if_t<std::is_base_of_v<condition_t, U>, all<Ts..., U>*> {
    return {};
  }
}
...
namespace props {
  ...
  struct empty_t : crtp_address_to_nullptr<empty_t>, property_t {
    template<class>
    using type = properties::properties<>;
  } constexpr *empty{};

  template<class K, template<class> class Tr>
  struct set_t : crtp_address_to_nullptr<set_t<K, Tr>>, property_t, conditions::condition_t {
    template<class V>
  ...
  template<class K, class V>
  struct fix_t : crtp_address_to_nullptr<fix_t<K, V>>, property_t {
    template<class>
  ...
  template<class ...Ts, class U>
  constexpr auto operator+(const multi<Ts...>*, const U&)
        -> std::enable_if_t<std::is_base_of_v<property_t, U>, multi<Ts..., U>*> {
    return {};
  }
...
namespace collect_values {
  struct na_t {} constexpr na{};
  struct adj_list_t {} constexpr adj_list_v{};
  struct adj_mat_t {} constexpr adj_mat_v{};
  struct edge_list_t {} constexpr edge_list_v{};
...
namespace collect_props {
  ...
  struct settable_prop {
    ...
    template<class U>
    constexpr auto operator==(U const&) const
      -> std::enable_if_t<(std::is_same_v<U, Acceptable> || ...), conditions::is_t<T, U>> {
      return {};
    }

    template<class U>
    constexpr auto operator=(U const&) const
      -> std::enable_if_t<(std::is_same_v<U, Acceptable> || ...), props::fix_t<T, U>> {
      return {};
    }
  };
  ...
  constexpr settable_prop<struct type_t, collect_values::adj_list_t, collect_values::adj_mat_t, collect_values::edge_list_t> type{};
...
namespace state_machine {
  ...
  struct any_of {
    ...
    constexpr static auto valid() {
      if constexpr ((static_cast<bool>(States::template valid<T, Props>()) + ...) == 1) {
        return std::tuple_element_t<0, decltype(std::tuple_cat(
                                             std::declval<std::conditional_t<States::template valid<T, Props>(), std::tuple<States>, std::tuple<>>>()...
                                             ))>::template valid<T, Props>();
      } else if constexpr ((static_cast<bool>(States::template valid<T, Props>()) + ...) > 1) {
        return assert_types::reason<multiple_good_recognition, assert_types::input<T, Props>, assert_types::at<CRTP>, decltype(std::tuple_cat(
                                                                                                                            std::declval<std::conditional_t<States::template valid<T, Props>(), std::tuple<decltype(States::template valid<T, Props>())>, std::tuple<>>>()...
                                                                                                                            ))>{};
      } else if constexpr ((static_cast<bool>(States::template valid_type<T>()) + ...) == 1) {
        return std::tuple_element_t<0, decltype(std::tuple_cat(
                                             std::declval<std::conditional_t<States::template valid_type<T>(), std::tuple<States>, std::tuple<>>>()...
                                             ))>::template valid<T, Props>();
      } else if constexpr ((static_cast<bool>(States::template valid_type<T>()) + ...) > 1 &&
                           (static_cast<bool>(States::template valid_conditions<T, Props>()) + ...) == 1) {
        return std::tuple_element_t<0, decltype(std::tuple_cat(
                                             std::declval<std::conditional_t<States::template valid_conditions<T, Props>(), std::tuple<States>, std::tuple<>>>()...
                                             ))>::template valid<T, Props>();
        ...
        return assert_types::reason<no_good_recognition, assert_types::input<T, Props>, assert_types::at<CRTP>,
                                    assert_types::got<false, decltype(std::tuple_cat(
                                                                   std::declval<std::conditional_t<States::template valid_conditions<T>(), std::tuple<decltype(States::template valid<T, Props>())>, std::tuple<>>>()...
                                                                   ))>>{};
      } else if constexpr ((static_cast<bool>(States::template valid_type<T>()) + ...) > 1) {
        return assert_types::reason<no_good_recognition, assert_types::input<T, Props>, assert_types::at<CRTP>,
                                    assert_types::got<false, decltype(std::tuple_cat(
                                                                   std::declval<std::conditional_t<States::template valid_type<T>(), std::tuple<decltype(States::template valid<T, Props>())>, std::tuple<>>>()...
                                                                   ))>>{};
      } else {
        return assert_types::reason<no_good_recognition, assert_types::input<T, Props>, assert_types::at<CRTP>>{};
      }
  ...
  template<template<class> class Tr>
  struct edge_container : any_of_r<edge_container<Tr>, Tr,
  ...
  template<template<class> class Tr>
  struct node_container : any_of_r<node_container<Tr>, Tr,
  ...
  template<template<class> class Tr = T>
  struct adj_list_cont : any_of_r<adj_list_cont<Tr>, Tr,
                                 transition<range, no, empty, node<E>>,
                                 transition<map, no, empty, node<K>, edge<V>>> {};

  template<template<class> class Tr = T>
  struct adj_list_node_p : any_of_r<adj_list_node_p<Tr>, Tr,
  ...
  template<template<class> class Tr = T>
  struct adj_list_node : any_of_r<adj_list_node<Tr>, Tr,
  ...
  struct adj_list : any_of<adj_list,
  ...
  struct adj_mat_elem_opt : any_of_r<adj_mat_elem_opt, O,
                                    transition<range, &(multi_edge == t), &(multi_edge = t), edge<E>>,
                                    transition<any, &(multi_edge == f), &(multi_edge = f), edge<>>> {};

  template<template<class> class Tr>
  struct adj_mat_elem : any_of_r<adj_mat_elem<Tr>, Tr,
  ...
  template<template<class> class Tr>
  struct edge_list_elem : any_of_r<edge_list_elem<Tr>, Tr,
  ...
  template<template<class> class Tr = T>
  struct graph_p : any_of_r<graph_p<Tr>, Tr,
